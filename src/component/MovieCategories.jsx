...
const MovieCategories = ({ movies, allCategories, onMovieClick }) => {
  ...
  const showAllCategories = allCategories && Object.keys(allCategories).length > 0;
      ...
      <div className="relative overflow-hidden rounded-lg">
        {movie.Poster && movie.Poster !== "N/A" ? (
          <img
            src={movie.Poster}
            alt={movie.Title}
            className="w-full h-52 sm:h-64 object-cover transform group-hover:scale-110 transition-transform duration-300"
          />
        ) : (
  ...
  const renderCategory = (categoryName, categoryMovies) => (
    <div key={categoryName} className="mb-8">
      <h2 className="text-2xl font-bold mb-4 text-white">{categoryNames[categoryName] || categoryName}</h2>
      <motion.div 
    ...
    return (
      <div className="mt-8">
        {Object.keys(allCategories).map(categoryName => (
          allCategories[categoryName] && allCategories[categoryName].length > 0 &&
